...
class ConfigurationTauxResourceIT {
    ...
    @Test
    @Transactional
    void checkInvalidIsRequired() throws Exception {
        int databaseSizeBeforeTest = configurationTauxRepository.findAll().size();
        // set the field null
    ...
    @Test
    @Transactional
    void putExistingConfigurationTaux() throws Exception {
        // Initialize the database
        configurationTauxRepository.saveAndFlush(configurationTaux);
    ...
    void deleteConfigurationTaux() throws Exception {
        ...
        restConfigurationTauxMockMvc
            .perform(delete(ENTITY_API_URL_ID, configurationTaux.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
